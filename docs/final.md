...
With the greedy search algorithm, the agents will calculate which is the closest note starting with one agent and then the next agent will take the next closest noteblock location until the either the amount of agents or the amount of noteblocks in the next interval run out. This algorithm is what we plan to be the most efficient of our algorithms. Compared to the other methods, greedy search takes the least amount of time to almost always find the most optimal solution, and if not, the algorithm will still find a very good solution. When there are more agents, the amount of time saved is much more apparent. This method would also need less memory than brute force since it does not go through each possible answer. 
